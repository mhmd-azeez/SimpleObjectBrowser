...
public static class CredentialManager
{
    ...
    {
        string applicationName = Marshal.PtrToStringAnsi(credential.TargetName);
        string userName = Marshal.PtrToStringAnsi(credential.UserName);
        string secret = null;

        string comment = null;
        if (credential.Comment != IntPtr.Zero)
        {
            comment = Marshal.PtrToStringAnsi(credential.Comment, (int)credential.Comment / 2);
        }

        if (credential.CredentialBlob != IntPtr.Zero)
        {
            secret = Marshal.PtrToStringAnsi(credential.CredentialBlob, (int)credential.CredentialBlobSize / 2);
        }
    ...
    {
        var zipped = Zip(secret);
        secret = Encoding.ASCII.GetString(zipped);

        byte[] byteArray = secret == null ? null : Encoding.ASCII.GetBytes(secret);
        ...
        credential.Comment = Marshal.StringToCoTaskMemAnsi("zip");
        ...
        credential.TargetName = Marshal.StringToCoTaskMemAnsi(applicationName);
        credential.CredentialBlob = Marshal.StringToCoTaskMemAnsi(secret);
        credential.UserName = Marshal.StringToCoTaskMemAnsi(userName ?? Environment.UserName);
    ...
    [DllImport("Advapi32.dll", EntryPoint = "CredReadW", CharSet = CharSet.Ansi, SetLastError = true)]
    static extern bool CredRead(string target, CredentialType type, int reservedFlag, out IntPtr credentialPtr);

    [DllImport("Advapi32.dll", EntryPoint = "CredWriteW", CharSet = CharSet.Ansi, SetLastError = true)]
    static extern bool CredWrite([In] ref CREDENTIAL userCredential, [In] UInt32 flags);

    [DllImport("advapi32", SetLastError = true, CharSet = CharSet.Ansi)]
